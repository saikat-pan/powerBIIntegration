...
namespace PowerBIEmbedded_AppOwnsData.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            employees.Add(new Employee() { Id = 1, EmployeeName = "Andrew Ma", IsSelected = false });
            employees.Add(new Employee() { Id = 2, EmployeeName = "Annelie Zubar", IsSelected = false });
            employees.Add(new Employee() { Id = 3, EmployeeName = "Carlos Grilo", IsSelected = false });
            employees.Add(new Employee() { Id = 4, EmployeeName = "Tina Lassila", IsSelected = false });
            employees.Add(new Employee() { Id = 5, EmployeeName = "Valery Ushakov", IsSelected = false });
        ...
        [HttpPost]
        public async Task<ActionResult> Index(EmbedConfig embededConfig)
        {
            ...
            //ViewBag.Employee=embededConfig.Employees.se
            foreach (var item in embededConfig.Employees)
            {
                if (item.IsSelected)
                    ViewBag.Employee = item.EmployeeName;
            }
            ViewBag.Message = "Render";
            ...
            using (var client = new PowerBIClient(new Uri(ApiUrl), tokenCredentials))
            {
                ...
                foreach (Report name in reports.Value)
                {
                    reportNames.Add(name.Name);
                }
                if (report == null)
                {
                    return View(new EmbedConfig()
                ...
                //employees.Add(new Employee() { EmployeeName = "Andrew Ma", IsSelected = false });
                //employees.Add(new Employee() { EmployeeName = "Annelie Zubar", IsSelected = false });
                //employees.Add(new Employee() { EmployeeName = "Carlos Grilo", IsSelected = false });
                //employees.Add(new Employee() { EmployeeName = "Tina Lassila", IsSelected = false });
                //employees.Add(new Employee() { EmployeeName = "Valery Ushakov", IsSelected = false });
                // Generate Embed Configuration.
                var embedConfig = new EmbedConfig()
                {
                    ...
                    Employees = embededConfig.Employees
                };
